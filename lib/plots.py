...
def get_sdf(decoder,path,epoch, shapename,resolution,mc_value,is_uniform_grid,verbose,save_html,save_ply,overwrite,t, points=None, with_points=False, latent=None, connected=False):
    filename = '{0}/igr_epoch_{1}_{2}'.format(path, epoch, shapename)
    ...
    if (not os.path.exists(filename) or overwrite):
        ...
        surface = get_surface_trace(points,decoder,latent,t,resolution,mc_value,is_uniform_grid,verbose,save_ply, connected)
    
    return surface['sdf']



def plot_surface(decoder,path,epoch, shapename,resolution,mc_value,is_uniform_grid,verbose,save_html,save_ply,overwrite,t, points=None, with_points=False, latent=None, connected=False, extension='obj'):
    ...
    if (not os.path.exists(filename) or overwrite):
        ...
        if (not surface['mesh_export'] is None):
            if extension == 'ply':
                ...
                surface['mesh_export'].export(filename + '.obj', 'obj')
                
        return surface['mesh_export']


def get_surface_trace(points,decoder,latent,t,resolution,mc_value,is_uniform,verbose,save_ply, connected=False):
    ...
    for i,pnts in enumerate(torch.split(grid['grid_points'],100000,dim=0)):
        ...
        if (not latent is None):
            pnts = torch.cat([latent.expand(pnts.shape[0], -1), pnts, t.unsqueeze(0).expand(pnts.shape[0],-1)], dim=1)
...
def plot_cuts_axis(points,decoder,latent,t,path,epoch,near_zero,axis,file_name_sep='/'):
    t = t.cuda()
    ...
    for index, pos in enumerate(position_cut):
        ...
        for i, pnts in enumerate(torch.split(field_input, 10000, dim=0)):
            if (not latent is None):
                pnts = torch.cat([latent.expand(pnts.shape[0], -1), pnts, t.unsqueeze(-1).repeat(pnts.shape[0], 1)], dim=1)
            z.append(decoder(pnts).detach().cpu().numpy())
        z = np.concatenate(z, axis=0)
...
def plot_cuts(points,decoder,path,epoch,near_zero,t, latent=None):
    t = t.cuda()
    ...
    for index, pos in enumerate(position_cut):
        ...
        for i, pnts in enumerate(torch.split(field_input, 1000, dim=-1)):
            input_=pnts
            if (not latent is None):
                input_ = torch.cat([latent.expand(pnts.shape[0],-1) ,pnts, t.unsqueeze(-1).repeat(pnts.shape[0], 1)],dim=1)
...
def get_surface_cut_trace(decoder,latent,t,resolution,mc_value, color, opacity, is_cut=False, is_cut_half=False, verbose=False):

    trace = []
    if is_cut:
        grid = get_grid_cut(resolution)
    elif is_cut_half:
        grid = get_grid_cut_half(resolution)
    else:
        grid = get_grid_uniform(resolution)
    ...
    if (not (np.min(z) > mc_value or np.max(z) < mc_value)):
        ...
        verts, faces, normals, values = measure.marching_cubes_lewiner(
            volume=z.reshape(grid['xyz'][1].shape[0], grid['xyz'][0].shape[0],
                             grid['xyz'][2].shape[0]).transpose([1, 0, 2]),
            level=mc_value,
            spacing=(grid['xyz'][1][2] - grid['xyz'][1][1],
                     grid['xyz'][0][2] - grid['xyz'][0][1],
...
def get_grid_cut(resolution):
    x = np.linspace(-1.2,1.2, resolution)
    y = x
    z = np.linspace(-1.2,0.0, int(resolution / 2))
...
def get_grid_cut_half(resolution):
    x = np.linspace(-1.2,1.2, resolution)
    y = x
    z = np.linspace(0,1.2, int(resolution / 2))
...
def plot_level_outside(path, epoch, decoder, latent, t, resolution, shapename):
    filename = '{0}/igr_epoch_{1}_{2}'.format(path, epoch, shapename)

    mc_value = 0.0
    rgba_list = ['rgba(220,226,233,1)',
                'rgba(184,180,169,1)',
                'rgba(203,165,141,1)',
                'rgba(207,142,112,1)',
                'rgba(200,111,81,1)',
                'rgba(180,69,52,1)']
    trace = get_surface_cut_trace(decoder,latent,t,resolution,mc_value=mc_value, color=rgba_list[0], opacity=1.0, is_cut=False)
    for i in range(1, 6):
        mc_value += 0.025
        trace = trace + get_surface_cut_trace(decoder,latent,t,resolution,mc_value=mc_value, color=rgba_list[i], opacity=1.0, is_cut=True)

    layout = go.Layout(title= go.layout.Title(text=shapename), width=1200, height=1200, 
                    scene=dict(xaxis=dict(range=[-1, 1], autorange=False,showgrid=False,showline=False,backgroundcolor="white",visible=False),
                                yaxis=dict(range=[-1, 1], autorange=False,showgrid=False,showline=False,backgroundcolor="white",visible=False),
                                zaxis=dict(range=[-1, 1], autorange=False,showgrid=False,showline=False,backgroundcolor="white",visible=False),
                                aspectratio=dict(x=1, y=1, z=1)))
    # ,paper_bgcolor='black'
    fig1 = go.Figure(data=trace, layout=layout)
    # fig2 = go.Figure(data=trace_surface2, layout=layout)
    offline.plot(fig1, filename=filename + '.html', auto_open=False)

def plot_level_inside(path, epoch, decoder, latent, t, resolution, shapename):
    filename = '{0}/igr_epoch_{1}_{2}'.format(path, epoch, shapename)
    mc_value = 0.0
    rgba_list = ['rgba(240,243,247,1)',
                'rgba(156,169,190,1)',
                'rgba(127,146,192,1)',
                'rgba(111,132,186,1)',
                'rgba(66,82,158,1)',
                'rgba(51,64,143,1)']
    trace = get_surface_cut_trace(decoder,latent,t,resolution,mc_value=mc_value, color=rgba_list[0], opacity=1, is_cut=True)
    trace = trace + get_surface_cut_trace(decoder,latent,t,resolution,mc_value=mc_value, color=rgba_list[0], opacity=0.4, is_cut_half=True)
    for i in range(1, 6):
        mc_value -= 0.02
        trace = trace + get_surface_cut_trace(decoder,latent,t,resolution,mc_value=mc_value, color=rgba_list[i], opacity=1.0, is_cut=True)
...
def get_mesh(path,epoch,decoder,latent,t,shapename,resolution, verbose=False,connected = False,mc_value = 0.0,extension='obj', return_sdf=False):
    filename = '{0}/igr_epoch_{1}_{2}'.format(path, epoch, shapename)
    meshexport = None
    grid = get_grid_uniform(resolution)
    z = []
    ...
    if extension == 'ply':
        ...
        meshexport.export(filename + '.obj', 'obj')
    if return_sdf:
        scio.savemat(filename + '.mat', {'sdf': volume})
        # np.save(filename+'.npy', volume)
