...
from lib.inside_mesh.triangle_hash import TriangleHash as _TriangleHash
...
def compute_iou(mesh_gt, mesh_pr, N=128):
    ''' compute iou score
        parameters
    ...
    mesh = trimesh.Trimesh(vertices=grid)
    grid = mesh.vertices

    occ_gt = torch.tensor(check_mesh_contains(mesh_gt, grid))
    occ_pr = torch.tensor(check_mesh_contains(mesh_pr, grid))
...
def compute_normal_consistency(mesh_gt, mesh_pr):
    ''' Computes normal consistency between mesh_gt and mesh_pr.
    Args:
        mesh_gt (trimesh.base.Trimesh): ground truth mesh
        mesh_pr (trimesh.base.Trimesh): predicted mesh
    '''
    points_src = mesh_gt.vertices
    normals_src = mesh_gt.vertex_normals
    points_tgt = mesh_pr.vertices
    normals_tgt = mesh_pr.vertex_normals
