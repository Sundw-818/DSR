...
specifications_filename = "model_config.json"
...
def load_experiment_specifications(experiment_directory):
    ...
    if not os.path.isfile(filename):
        raise Exception(
            ...
            + '"model_config.json"'.format(experiment_directory)
        )
...
def get_output_filename(reconstruction_dir, name):
    dir = os.path.join(reconstruction_dir, name.split("/")[-1].replace(".mat", ""))
    return dir
